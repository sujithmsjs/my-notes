...
-	insertMany	db.collection.insertMany([document1, document2, ...])
Read
-	find	db.collection.find(query)
-	count	db.collection.count(query)

-	distinct	db.collection.distinct(field, query)
	Ex:
		db.collection.distinct("country", { population: { $gt: 1000000 } })


Update	
-	replaceOne	db.collection.replaceOne(filter, replacement)
-	update	db.collection.update(filter, update, options)
-	findOneAndUpdate	db.collection.findOneAndUpdate(filter, update, options)
-	findOneAndReplace	db.collection.findOneAndReplace(filter, replacement, options)
-	findOneAndDelete	db.collection.findOneAndDelete(filter)
Delete
-	deleteOne	db.collection.deleteOne(filter)
	MongoDB will delete the first document that matches the specified filter.
-	deleteMany	db.collection.deleteMany(filter)
Indexes
-	createIndex	db.collection.createIndex(keys, options)
-	dropIndex	db.collection.dropIndex(index)
-	dropIndexes	db.collection.dropIndexes()
Aggregation
-	aggregate	db.collection.aggregate(pipeline)
-	group	db.collection.group(keys, condition, initial, reduce, finalize)




How to find Count Distinct Values?

var distinctCountries = db.getCollection('shift_roaster').distinct("shift_id");
var count = distinctCountries.length;
print(count);
...
$sum, $subtract $divide, $mod, $multiply, $sqrt, $pow
$round
...
db.students.aggregate([
  // Add a new field called "grade" based on marks
  {
    $addFields: {
      grade: {
        $switch: {
          branches: [
            ...
            { case: { $gte: [ "$marks", 50 ] }, then: "D" },
            { case: { $gte: [ "$marks", 35 ] }, then: "E" },
          ],
          default: "F"
        }
      }
  ...
  // Group students by their grades and count
  {
    $group: {
      _id: "$grade",
      count: { $sum: 1 }
  ...
  // Calculate the total number of students
  {
    $group: {
      ...
      results: { $push: { grade: "$_id", count: "$count" } }
    }
  },
  ...
  },
  {
    $project: {
      ...
      percentage: {
        $multiply: [
          { $divide: [ "$results.count", "$total_students" ] },
          100
        ]
      }
    }
...
db.getCollection('case_details').find({"updated_date" : { $gte : ISODate("2023-03-15T18:30:00.000Z"),  $lte : ISODate("2023-03-16T18:29:59.999Z")}})
